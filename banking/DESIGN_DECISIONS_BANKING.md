...
push a balance up. But we're a casino, and the whole point is that money also comes back *from* the players.
...
all three. Transactions work the same way, with `TransactionQueryService` + `TransactionManagementService`.
...
enum (`SLOTS`, `ROULETTE`) instead of a plain string. An unknown party can't even get into the system, because
...
10.50, and `.../deposit/10/5` is 10.05, so a single-digit `decimals` counts as that many cents (0.05), not
...
a transaction, though, only touch the ledger row, and they deliberately **don't** reverse the old amount or
...
transaction, because the deposit already committed in its own request by the time we `save`. So if the `save`
